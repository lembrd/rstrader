...
use tokio::sync::mpsc;
use types::{RawMessage, OrderBookL2Update};
...
impl MultiSubscriptionManager {
    ...
    pub async fn spawn_all_subscriptions(&mut self, processed_tx: mpsc::Sender<OrderBookL2Update>, verbose: bool) -> Result<()> {
        ...
        for (index, subscription) in self.subscriptions.iter().enumerate() {
            ...
            let mut connector: Box<dyn ExchangeConnector<Error = AppError>> = match subscription.exchange {
                Exchange::BinanceFutures => {
                    log::info!("[{}] Creating Binance Futures connector for {}", index, subscription.instrument);
            ...
            log::info!("[{}] Connecting to {:?} for {}...", index, subscription.exchange, subscription.instrument);
            connector.connect().await
                .map_err(|e| AppError::connection(format!("Failed to connect to {:?}: {}", subscription.exchange, e)))?;
            
            // Validate symbol exists
            log::info!("[{}] Validating symbol: {}", index, subscription.instrument);
            connector.validate_symbol(&subscription.instrument).await
                .map_err(|e| AppError::validation(format!("Invalid symbol {} on {:?}: {}", subscription.instrument, subscription.exchange, e)))?;
            
            // Get initial order book snapshot if L2 stream
            if matches!(subscription.stream_type, StreamType::L2) {
                log::info!("[{}] Getting initial order book snapshot for {} on {:?}", index, subscription.instrument, subscription.exchange);
                let _snapshot = connector.get_snapshot(&subscription.instrument).await
                    .map_err(|e| AppError::data(format!("Failed to get snapshot for {} on {:?}: {}", subscription.instrument, subscription.exchange, e)))?;
                log::info!("[{}] Successfully retrieved order book snapshot for {} on {:?}", index, subscription.instrument, subscription.exchange);
            }
            ...
            let connector_handle = tokio::spawn(async move {
                log::info!("[{}] Starting {:?} stream for {} on {:?}", connector_index, stream_type, connector_symbol, exchange);
                
                match stream_type {
                    StreamType::L2 => {
                        connector.start_depth_stream(&connector_symbol, raw_tx).await
                            .map_err(|e| AppError::stream(format!("L2 stream failed for {} on {:?}: {}", connector_symbol, exchange, e)))
                    }
                    StreamType::Trades => {
                        connector.start_trade_stream(&connector_symbol, raw_tx).await
                            .map_err(|e| AppError::stream(format!("Trade stream failed for {} on {:?}: {}", connector_symbol, exchange, e)))
                    }
                }
            });
        ...
        log::info!("Successfully spawned {} subscription pairs (connector + processor)", self.subscriptions.len());
        Ok(())
    }
    ...
    pub async fn wait_for_any_completion(&mut self) -> Result<()> {
        ...
        let (result, _index, remaining) = futures_util::future::select_all(self.connector_handles.drain(..)).await;
        ...
        match result {
            ...
            Err(e) => {
                ...
                Err(AppError::internal(format!("Subscription component task failed: {}", e)))
            }
        }
    }
    ...
    pub async fn shutdown(&mut self) {
        log::info!("Shutting down {} subscription component tasks", self.connector_handles.len());
...
async fn run_application(args: Args) -> Result<()> {
    ...
    if args.is_single_subscription_mode() {
        ...
        connector.validate_symbol(&symbol).await
        ...
        if matches!(stream, StreamType::L2) {
            ...
            let _snapshot = connector.get_snapshot(&symbol).await
        ...
        let stream_handle = {
            ...
            tokio::spawn(async move {
                ...
                match stream_type {
                    StreamType::L2 => {
                        connector.start_depth_stream(&symbol_clone, raw_tx).await
                            .map_err(|e| AppError::stream(format!("L2 stream failed: {}", e)))
                    }
                    StreamType::Trades => {
                        connector.start_trade_stream(&symbol_clone, raw_tx).await
        ...
        let subscriptions = args.get_subscriptions()
            .map_err(|e| AppError::cli(format!("Invalid subscriptions: {}", e)))?
            .unwrap();
        
        // Display configuration in verbose mode
        if args.verbose {
            log::info!("Starting xtrader with {} subscriptions:", subscriptions.len());
            for (i, sub) in subscriptions.iter().enumerate() {
                log::info!("  [{}] {:?}:{:?}@{}", i+1, sub.stream_type, sub.exchange, sub.instrument);
            }
            log::info!("  Output: {}", args.output_parquet.display());
        }
        ...
        manager.spawn_all_subscriptions(processed_tx, args.verbose).await?;
